...
module vorticity_tendency_mod
  ...
  use MPI
  use parcel_interpolation_mod, only: x_coords, y_coords, z_coords, grid2par, par2grid, divfree
...
!    !$OMP END SINGLE
    ...
    call divfree(current_state,dp,dq,dr,current_state%parcels%dpdt,current_state%parcels%dqdt,current_state%parcels%drdt)
